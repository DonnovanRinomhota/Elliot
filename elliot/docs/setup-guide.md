...
5. **Add billing credits.** console.anthropic.com → Plans & Billing → add credits. A fresh account or an expired trial balance will make every "Call Claude" node fail with `Bad request - please check your parameters` / "Your credit balance is too low" — this looks like a config bug but isn't one; check billing first if this specific error shows up. This is the single most important node in the whole workflow: it's the actual reasoning step everything else (tenant resolution, RLS, conversation logging, the tool-approval gate) exists to feed and log around. A few dollars comfortably covers all dev/test usage at this stage — this isn't a subscription, it's metered by actual usage.

**Check:** open "Call Claude (turn 1)" and "Execute step" with a manually-crafted test input — you should get back a real Claude API response object, not a 401 or a billing error.
